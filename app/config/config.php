...
define('DB_PASSWORD', '');
define('DB_DATABASE', 'db_ecommerce');

// API Configurations
define('EXCHANGE_RATE_API_URL', 'https://open.er-api.com/v6/latest/EUR');
...
if (isset($_POST['reg_user'])) {
  ...
  $birthdate = '';
  if (!empty($b_date) && !empty($b_month) && !empty($b_year)) {
    $birthdate = sprintf('%04d-%02d-%02d', $b_year, $b_month, $b_date);
  } else {
  ...
  if (count($errors) == 0) {
    ...
    $query = "INSERT INTO register (Name, email, password, birthdate) 
...
// Include separated architectural modules
require_once __DIR__ . '/../services/currency_service.php';
